...
struct UserService {
    ...
    static func paginateUsers(lastCursor: String?, pageSize: UInt) async throws -> UserNode {
        ...
        guard let firts = mainSnapshot.children.allObjects.first as? DataSnapshot,
        ...
        if users.count == mainSnapshot.childrenCount {
            ...
            let userNode = UserNode(users: filteredUsers, currentCursor: firts.key )
            return userNode
        }
...
//            
//            if users.count == mainSnapshot.childrenCount {
//                let userNode = UserNode(users: users, currentCursor: firts.key )
//                return userNode
//            }
//            
//            return .emptyNode
